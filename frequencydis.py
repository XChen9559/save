...
class MultiFrequencyDiscriminator(nn.Module):
    def __init__(self, nch, window):
        ...
        self.discriminators = nn.ModuleList([FrequencyDiscriminator(nch * 2, self.hidden_channels) for _ in range(len(self.window))])

    def forward(self, est, sample_rate=32000):
        if est.dim() < 3:
            est = est.unsqueeze(1)
        B, nch, _ = est.shape
        assert nch == self.nch
        ...
        for i in range(len(self.discriminators)):
            ...
            if torch.isnan(est_RI).int().sum() != 0:
                pass
            valid_enc = int(est_RI.shape[2] * sample_rate / sample_rate)
            ...
            if torch.isnan(est_out).int().sum() != 0:
                arr = torch.where(torch.isnan(est_out))
                pass
            #if torch.any(torch.isnan(est_feat_map[0])):
            #    pass
