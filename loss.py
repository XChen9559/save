...
class Stft(nn.Module):
    def __init__(self, n_fft: int, hop: Optional[int] = None, window: Optional[Tensor] = None):
        ...
        elif n_fft==1024 and hop==640:
            win_hop640 = torch.sin(torch.Tensor(np.pi * np.array(range(384)) / (384 - 1)) / 2)
            window = torch.cat([win_hop640, torch.ones(1024 - 2 * 384), win_hop640.flip(0)], 0).to('cuda')
        else:
...
class Disc_Loss(_Loss):
    def __init__(self, sample_rate=32000, eps=1e-8):
        super(Disc_Loss, self).__init__()
        self.sr = sample_rate
        self.PESQ_Loss = PesqLoss(0.5, sample_rate=self.sr).cuda()
        self.D_Loss = MultiFrequencyDisLoss()
        

    def forward(self, target_Disc, ested_Disc, target, ested):
        l_disc = self.D_Loss(target_Disc, ested_Disc)
        l_pesq = self.PESQ_Loss(target, ested)
        l_pesq = -torch.mean(l_pesq)

        return l_pesq + l_disc

class Gen_FM_Loss(_Loss):
    def __init__(self, eps=1e-8):
        super(Gen_FM_Loss, self).__init__()
        self.eps = eps
        self.f_FM = 50
        self.f_fake = 1

    def forward(self, est_outputs, target_outputs, est_Feat_map, target_Feat_map):
        G_fake = 0
        feature_matching = 0
        for i in range(len(est_outputs)):
            G_fake = G_fake + (est_outputs[i] - 1).pow(2).mean() / len(est_outputs)
            for j in range(len(est_Feat_map[i])):
                feature_matching = feature_matching + \
                                   (est_Feat_map[i][j] - target_Feat_map[i][j].detach()).abs().mean() / (
                                               target_Feat_map[i][j].detach().abs().mean() + self.eps)
        feature_matching = feature_matching / (len(est_outputs) * len(est_Feat_map[0]))
        
        
        total_loss = self.f_fake * G_fake + self.f_FM * feature_matching 
        #print(self.f_fake * G_fake,self.f_FM * feature_matching )
        return total_loss

class DNSMOS_Loss(_Loss):
    def __init__(self, sr=32000, target_sr=16000):
        super(DNSMOS_Loss, self).__init__()
        self.sr = sr
        self.target_sr = target_sr
        primary_model_path = os.path.join('df/DNSMOS', 'sig_bak_ovr.onnx')
    ...
    def forward(self, target, ested):
        ...
        #target = target
        #ested = ested
        len_samples = int(INPUT_LENGTH * self.target_sr)
        for i in range(target.shape[0]):
            target_mos_ovr_seg = []
            ested_mos_ovr_seg = []
            target_i = target[i].cpu().numpy()
            ested_i = ested[i].cpu().numpy()
            #num_hops = int(np.floor(target_i.shape[0] / self.sr) - INPUT_LENGTH) + 1
            target_i = librosa.resample(target_i, orig_sr=self.sr, target_sr=self.target_sr)
            ested_i = librosa.resample(ested_i, orig_sr=self.sr, target_sr=self.target_sr)
            while len(target_i) < len_samples:
                target_i = np.append(target_i, target_i)
            while len(ested_i) < len_samples:
                ested_i = np.append(ested_i, ested_i)
            num_hops = int(np.floor(len(target_i.shape) / self.target_sr) - INPUT_LENGTH) + 1
            for idx in range(num_hops):
                target_seg = target_i[int(idx * self.sr): int((idx + INPUT_LENGTH) * self.sr)]
                target_features = np.array(target_seg).astype('float32')[np.newaxis, :]
                target_oi = {'input_1': target_features}
                xxx, xxx2, target_mos_ovr = self.onnx_sess.run(None, target_oi)[0][0]
                target_mos_ovr_seg.append(target_mos_ovr)

                ested_seg = ested_i[int(idx * self.sr): int((idx + INPUT_LENGTH) * self.sr)]
                ested_features = np.array(ested_seg).astype('float32')[np.newaxis, :]
                ested_oi = {'input_1': ested_features}
                _, _, ested_mos_ovr = self.onnx_sess.run(None, ested_oi)[0][0]
                ested_mos_ovr_seg.append(ested_mos_ovr)

            ested_mos_ovr = np.mean(ested_mos_ovr_seg)
            target_mos_ovr = np.mean(target_mos_ovr_seg)

            target_mos.append(target_mos_ovr)
            ested_mos.append(ested_mos_ovr)

        target_mos_v = torch.Tensor(target_mos)
        ested_mos_v = torch.Tensor(ested_mos)
        loss_mos = F.mse_loss(target_mos_v, ested_mos_v)
        return loss_mos
